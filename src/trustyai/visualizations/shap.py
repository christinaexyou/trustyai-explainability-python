...
# pylint: disable = consider-using-f-string, too-few-public-methods, missing-final-newline
...
from src.trustyai.visualizations.visualization_results import VisualizationResults
...
class SHAPViz(VisualizationResults):
    ...
    def _matplotlib_plot(self, explanations, output_name=None, block=True, call_show=True) -> None:
        """Visualize the SHAP explanation of each output as a set of candlestick plots,
        one per output."""
        with mpl.rc_context(drcp):
            shap_values = [
                pfi.getScore()
                for pfi in explanations.saliency_map()[output_name].getPerFeatureImportance()[
                    :-1
                ]
            ]
            feature_names = [
    ...
    def _get_bokeh_plot_dict(self, explanations, output_name):
        return {
            decision: self._get_bokeh_plot(explanations, output_name)
